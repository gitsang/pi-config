...
| **pi-status** | Mirrors Pi state onto the tmux window tab and each Pi footer. The tmux marker aggregates processes by PID across the whole window (including multiple terminals in one pane): all idle → one idle glyph; otherwise non-zero generating/done states → icon plus superscript count. |
