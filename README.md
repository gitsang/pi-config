...
| **pi-settings-lock** | Prevents in-session model/thinking switches from overwriting the global `defaultModel`/`defaultProvider`/`defaultThinkingLevel` in `settings.json` (restores the snapshot taken at session start). `enable: true`. `/settings-lock [status]`, `/settings-lock model save` / `/settings-lock thinking save` persist current as new default. |
...
`pi-command-panel`, `pi-auto-title`, and `pi-settings-lock` are independent
UX/behavior tweaks.
