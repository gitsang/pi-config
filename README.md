...
| **pi-settings-lock** | Prevents in-session model/thinking switches from overwriting the global `defaultModel`/`defaultProvider`/`defaultThinkingLevel` in `settings.json` (restores the snapshot taken at session start). `enable: true`. `/settings-lock` status, `/settings-save` persists current as new default. |
