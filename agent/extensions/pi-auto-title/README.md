...
Reads `config.json` next to this extension; project override: `<cwd>/.pi/pi-auto-title.json` (trusted projects only). See `config.example.json` for all fields (`model`, `onFirstTurn`, `onCompact`, `refreshEveryTurns`, `maxTitleLength`, `language`, `setTerminalTitle`).
