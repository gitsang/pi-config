...
- `/statusline-reset` — clear TTFT/TPS/prefill/decode/task-elapsed history.
...
Configs deep-merge over built-in defaults. Define `lines` and `modules`; modules use sources such as `session.cwd`, `model`, `thinking`, `usage.*`, `ctx.*`, `task.elapsed`, `task.elapsedTotal`, `ttft`, `tps`, `prefill`, `decode`, `focus`, `ext-status`, and `literal`. See `config.example.json` for the full template.

Adding `"breakdown": true` to an elapsed module appends session totals as `100s(25s/33s)` — prefill (request start → first token) / decode (first token → message end). Both windows exclude tool execution, so the pair adds up to response time, not `task.elapsedTotal`. `breakdownColor` (default `comment`) colors the parentheses.
